...
class Models_MThanhvien extends Project
{
	...
	function checklogin($username,$password){
		$pass = md5($password);
		$sql = "SELECT * FROM mn_user WHERE username= '$username' AND password = '$pass'";
		$kq = mysql_query($sql) or die(mysql_error());
		return $kq; 
	}
	function checkEmail($email){
		$sql = "SELECT * FROM mn_user WHERE email= '$email'";
		$kq = mysql_query($sql) or die(mysql_error());
		return $kq; 
	}
	...
	function checkUSername($username){
		$sql = "SELECT * FROM mn_user WHERE username= '$username'";
		$kq = mysql_query($sql) or die(mysql_error());
		$ks = mysql_fetch_row($kq);
		return $ks[0]; 
	}
	function checkEmailEdit($email,$id){
		$sql = "SELECT * FROM mn_user WHERE email= '$email' AND id <>'$id'";
		$kq = mysql_query($sql) or die(mysql_error());
		return $kq; 
	}
	/*
