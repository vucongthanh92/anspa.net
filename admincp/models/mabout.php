...
class Models_MAbout extends Project
{
	...
	function listdata($where = "",$start,$numrow){
		if($where != "")
			$this->where($where);	
		$this->getdata(TBL_ABOUT,"sort ASC","$start,$numrow");
		$row = $this->num_rows();
	...
	function sortData($data){
		foreach($data as $key => $value){
			$this->where("Id = $key");
			$this->update(TBL_ABOUT,"sort = $value");
		}
	}
}
?>