...
class Models_MBacsy extends Project
{
	...
	function listdata($where = "",$start,$numrow){
		if($where != "")
			$this->where($where);	
		$this->getdata(TBL_BACSY,"sort ASC, Id desc","$start,$numrow");
		$row = $this->num_rows();
	...
	function sortData($data){
		foreach($data as $key => $value){
			$this->where("Id = $key");
			$this->update(TBL_BACSY,"sort = $value");
		}
	}
}
?>