...
class Models_MPicture extends Project
{
	...
	function listdata($where = "",$start,$numrow){
		if($where != "")
			$this->where($where);	
		$this->getdata(TBL_PICTURES,"sort asc,Id asc","$start,$numrow");
		$row = $this->num_rows();
	...
	function del_allcheck($data){
		...
		{
			$this->delete(TBL_PICTURES,'Id = '.$item);
		}
	}
	...
	function del_onecheck($id){
		$this->delete(TBL_PICTURES,'Id = '.$id);
	}
	...
	function sortData($data){
		foreach($data as $key => $value){
			$this->where("Id = $key");
			$this->update(TBL_PICTURES,"sort = $value");
		}
	}
}
?>