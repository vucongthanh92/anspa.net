...
class Models_MThietbi extends Project
{
	...
	function listdata($where = "",$start,$numrow){
		if($where != "")
			$this->where($where);	
		$this->getdata(TBL_THIETBI,"sort ASC,idcat DESC","$start,$numrow");
		$row = $this->num_rows();
	...
	function del_allcheck($data){
		...
		{
			$this->delete(TBL_THIETBI,'Id = '.$item);
		}
	}
	...
	function sortData($data){
		foreach($data as $key => $value){
			$this->where("Id = $key");
			$this->update(TBL_THIETBI,"sort = $value");
		}
	}
}
?>