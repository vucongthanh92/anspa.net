...
<h1><img src = '<?php echo ADMIN_PATH_IMG;?>s_db.png'> Sửa nhóm thiết bị</h1>
<br/>
<hr/>

<form action = '<?php echo BASE_URL_ADMIN;?>nhomthietbi/edit/<?php echo $data['info']['Id']?>' method = 'post' enctype = "multipart/form-data">
<table>
    ...
    <tr>
        ...
        <select name="parentid">
			 ...
			 ?>
             <option <?php if($row['Id']==$data['info']['parentid']) echo "selected";?> value="<?=$row['Id']?>"><?=$row['title_vn']?></option>
             <?php } ?>
        </select>
        </td>
	</tr>
    ...
    <tr>
		...
		<td><input type = 'text' name = 'price' value="<?php echo $data['info']['price']?>" onkeyup="this.value = FormatNumber(this.value);"> VNĐ</div></td>
	</tr>
